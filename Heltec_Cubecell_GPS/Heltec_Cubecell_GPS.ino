...
#include "GPS_Air530Z.h"            // Biblioteca do GPS on board https://github.com/HelTecAutomation/CubeCell-Arduino
...
#define devEUI "00:00:00:00:02:02:02:02"  // Endereço de identificação do dispositivo
#define boardName "Cubecell GPS-6502"     // Nome da placa utilizada
...
void OnRxDone( uint8_t *payload, uint16_t size, int16_t rssi, int8_t snr ){
  ...
  if(isLost){
    state = SEND_LOCATION;
    time_to_sleep = TIME_TO_SLEEP_LOST;
  }
  else{
...
void setup() {
  ...
  GPS.begin();                      // Inicialização da comunicação Serial com o GPS
  txNumber=0;
  Rssi=0;
